// Package cli assembles the tc command tree from the topcoder
...
// NewApp assembles the kit application from the topcoder domain. The
...
//
// To add a command, declare it in topcoder/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := topcoder.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(topcoder.Domain{}).Register(app)
