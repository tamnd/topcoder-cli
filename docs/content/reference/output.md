...
describes what users get. Pick a format with `-o`, or let tc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tc <command> -o table   # aligned columns for reading
tc <command> -o jsonl   # one JSON object per line, for piping
tc <command> -o json    # a single JSON array
tc <command> -o csv     # spreadsheet friendly
tc <command> -o tsv     # tab-separated
tc <command> -o url     # just the URL column
tc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tc <command> --fields id,title,url
```
...
```bash
tc <command>            # a table, because this is a terminal
tc <command> | wc -l    # JSONL, because this is a pipe
```
